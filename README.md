...
- **`index.html`** — the full single-couple wedding website:
  Nav · Hero (live countdown) · Hikâyemiz (story) · Detaylar (details + program + venue) ·
  Anılarımız (gallery) · LCV (interactive RSVP) · Footer.
...
- **RSVP** is front-end only (shows a thank-you state). Wire the `<form>` submit in `js/app.js` to a real endpoint to collect responses.
